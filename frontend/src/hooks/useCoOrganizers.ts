...
export function useCoOrganizers(activityId?: string) {
    ...
    const addCoOrganizer = useCallback(
        async (userId: string, activity: Activity): Promise<Activity> => {
            ...
            try {
                ...
                const updated = await pb.collection('activities').update<Activity>(
                    activityId,
                    {
    ...
    const getAvailableUsers = useCallback(
        async (activity: Activity): Promise<User[]> => {
